...
双文本上线后，问题重心变了。大多数正确 chunk 已经能进候选池，接下来要处理的是排序：候选池里谁该排在前面，谁只是看起来相关。
...
这些数字说明 top20 召回已经很高。继续堆召回，短期收益有限；更值得先处理的是候选排序、重复 chunk 和干扰项。

旧 M5 设计把文本 reranker、表格 cell 定位、引用图扩展、LLM 元数据和 token 预算放在同一个阶段。范围太大，做完以后也很难判断到底是哪一块带来了收益。因此新的 M5 先拆出 reranker 主线：
...
表格 cell 定位、引用图扩展、LLM 离线元数据和 token 预算仍然属于 M5 后续子阶段，本 spec 先不碰。

## 2. 目标

M5.1 先把 reranker 这条链路搭起来：

1. 在 RRF 之后增加一个可配置的 reranker 阶段；
2. 约定 reranker 的输入、输出和回退行为；
3. 实现 `LightReranker`，用可解释规则提升排序并降低重复；
4. 让普通问题里的 `key_terms` 以软信号参与排序；
5. 保留条款号、方法号、标准号这类硬信号的保护逻辑；
...
3. 与 `LightReranker` 做 A/B，对比它是否真的改善排序。
...
M5.2 只负责接入大模型 reranker，不把表格、引用图、元数据一起带进来。
...
`method_no` 和 `clause_no` 继续硬置顶。只要用户明确问了编号，模型和轻量规则都不能把命中的 chunk 压下去。

## 4.1 M5.1 实现决策（2026-06-30 brainstorm 确认）

1. **作用范围**：LightReranker 仅作用于普通题（`GeneralFact` / `ClauseLookup` / `MethodLookup`）。列举题（`ListByCondition`）直接 bypass reranker，完全保留现有「`key_terms` 直查补召回 + `demote_without_keyterms`」覆盖链路（压力机/万能/马歇尔 coverage 已近满分，动它回归风险最高、收益最低）。
2. **打分形态**：以 RRF 排名为基底做**小步加/减分微调**，同分保留 RRF 原序（稳定排序）；**不**另算独立分对整池重排，避免把噪声推前（呼应 §13 风险1）。signal 权重先给保守默认，靠 rich eval 调。
3. **候选回查批量化**：`RerankCandidateBuilder` 用**批量回查**（新增 `PgClient::get_chunks(ids)`，单条 `WHERE chunk_id = ANY($1)`），不逐个 `get_chunk`——后者每次新建 pqxx 连接，`candidate_pool_k`≈`top_k*4` 时在线与 eval 都会显著变慢。
4. **默认值与开关**：`rerank.mode` 默认 `light`，但是否保留该默认由 off-vs-light 的 rich eval 结果决定；若关键指标回归则退回 `off`。M5.1 只实现 `off` 与 `light`。
5. **pin 顺序**：`method_no` / `clause_no` 硬 pin 放在 `RerankPolicy` 截断**之后**、且在候选池上取，确保硬命中不被 rerank/截断挤掉。

## 5. 组件

### 5.1 RerankCandidate

`Candidate` 现在只有 `standard_id`、`chunk_id`、`score` 和 `source`，不够做内容重排。M5.1 新增内部结构 `RerankCandidate`，由 PG 回查补齐字段：
...
这是 reranker 的输入结构，不替代数据库里的 `RetrievalChunkRow`。

### 5.2 RerankCandidateBuilder

`RerankCandidateBuilder` 把 RRF 输出的 `Candidate` 转成 `RerankCandidate`：
...
这样排序器只关心排序，不需要知道数据从哪里来。

### 5.3 IReranker

M5.1 引入一个小接口，轻量规则和大模型都走它：
...
1. 输入顺序是 RRF 顺序；
2. 输出仍是 `Candidate`，方便接回现在的 `text_retrieve`；
3. reranker 不负责重新召回；
...
`LightReranker` 是 M5.1 的默认实现，用确定性规则打分。它不负责替代大模型，只负责先把排序框架和可解释信号跑通。
...
2. 只有泛词命中、没有核心词命中时，不给强加分；
3. `source` 单一路径且原始 RRF 排名靠后时，不因为单个弱词命中大幅上浮。

普通问题的 `key_terms` 只做软加分。列举题继续使用现有的“关键词直查补召回 + 无关键词候选下压”逻辑。

### 5.5 RerankPolicy

`RerankPolicy` 处理排序后的保护和截断：
...
M5.1 可以先把 `method_no` 和 `clause_no` pin 留在 `text_search.cpp` 现在的位置，避免一次改太多。设计上它们属于 policy 层，M5.2 前可以再把边界收回来。
...
`candidate_pool_k` 控制 RRF 输出给 reranker 的候选池大小。现在非列举题会直接把 RRF 截断到 `top_k`，M5.1 需要把池子放大一点，否则 reranker 没有调整空间。
...
这样做可以避开一个常见误伤：用户问题本来就模糊，关键词没命中，不代表候选就是错的。普通问题用软加分就够了。
...
M5.2 不预设模型一定比 light 好。`model` 或 `hybrid` 能不能默认启用，要看 rich eval 的结果。
...
| 框架一次改太多 | M5.1 先保留现有 method/clause pin 位置，后面再收敛到 policy |
...
RRF 负责把三个人找来的资料合并。但 RRF 只知道“这个资料在某一路排第几”，不会细看资料内容。
